...
Default prefix is **Ctrl+b**; this config uses **Ctrl+s**.

- Press **Ctrl+s** then a command key (e.g. **Ctrl+s** then **%** for a vertical split).

## First run — install plugins

1. Open tmux (e.g. **Super+Enter** from Hyprland).
2. Press **Ctrl+s** then **Shift+i** (capital **I**) — **TPM** installs plugins.

Plugins include **tmux-fzf** for fuzzy session switching.

## Useful keys (after prefix Ctrl+s)
...
**Ctrl+s t** opens **`my-tmux-sessionizer`** — fzf over `~/repos`, `~/projects`, `~/code`, `~/dev`, `~/src`; creates a named tmux session per folder with nvim + shell window. Adjust paths in `scripts/my-tmux-sessionizer` if needed.
